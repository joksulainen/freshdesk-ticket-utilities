...
def getTicketList():
    ...
    loadedTicketlist = backend.getTicketList(1, fromDate=fromDate, untilDate=untilDate)
    for x in range(len(loadedTicketlist)):
        ticket = loadedTicketlist[x]
        try:
...
u0.place(x=5, y=0)
sel_company = ttk.Combobox(frm_ticketlist, values='""', width=20)
...
btn_refreshlist = ttk.Button(frm_ticketlist, text='Refresh', command=getTicketList)
btn_refreshlist.place(x=350, y=45)
...
u4 = ttk.Label(frm_ticket, text='Description')
u4.place(x=236, y=0)
...
u5 = ttk.Label(frm_ticket, text='Ticket ID')
u5.place(x=5, y=0)
